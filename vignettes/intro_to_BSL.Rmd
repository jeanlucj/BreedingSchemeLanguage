...
Define the genetic architecture of the plant species and other whole simulation settings.

```{r}
simEnv <- defineSpecies(nSim = 5)
```

- Here, we plan to conduct five simulations using default settings (i.e., genome with seven chromosomes of 150 cM length, historical effective population size of 100, trait controlled by additive QTLs, etc.). 

Generating the historical haplotypes is rather slow, but they are saved and can be reused:
...
Estimate the *per se* or own performance of individuals in the initial breeding population.
...
Select using pedigree information
...
- The thick line shows the mean value of the five simulations, while each thin line shows the selection responses in each simulation.
